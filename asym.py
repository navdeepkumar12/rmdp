...
plt.plot(rTs)
plt.plot([1 for s in rTs])
plt.legend(['U^s_{}'.format(p), 'non-robust'])
plt.ylabel('Relative time')
plt.xlabel('Number of states')
plt.title('Relative time for S-rect L_{} robust, A={}'.format(p,pm.A))
plt.savefig('Us_{}_A{}'.format(p,pm.A))
np.savetxt('rTs_Us_{}_A{}'.format(p,pm.A))
print('\n\n NEW EXPERIMENT \n\n')
